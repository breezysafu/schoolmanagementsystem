...
if( 'POST' == $_SERVER['REQUEST_METHOD'] && isset($_POST['noh']) && $_POST['noh'] != '') {
	...
	$sessions = $wpdb->get_results("SELECT * from $wh_table");
		if(empty($sessions)){
			echo "<div class='alert alert-danger'>No Working Hours added. Please add at <a href='admin.php?page=sch-settings&sc=WrkHours'>Settings</a></div>";
			return false;
		}
	?>
	<div class="box box-info">
		...
		<div class="box-body">
			<form name="" method="post" class="form-horizontal">
				<input type="hidden" name="wpsp_class_name" value="<?php echo $wpsp_class_id; ?>">
				...
				<div class="form-group">
					<div class="col-md-offset-4">
						<input type="submit" name="last-step" value="submit" class="btn btn-primary">
					</div>
				</div>
			</form>
		</div>
	...
	<div class="box box-info">
		...
		<div class="box-body">
			...
			if(count($clt)==0) {
				...
				No Subjects retrieved, Check you have subject for this class at <a href='".admin_url('admin.php?page=sch-subject')."'>Subjects</a></div>";
			}
			if ($error == 0) {
				$timetable=array();
				...
				<div class="row"> 
								<div class="col-md-12">
	<label>
					Class : <?php echo wpsp_GetClassName(sanitize_text_field($_POST['wpsp_class_name'])); ?>	</label>
				...
				<div class="right table-responsive" id="TimetableContainer">
					...
					<div class="col-md-12 col-lg-12"><span class="pull-right"><a href="javascript:;" class="btn btn-danger" id="deleteTimetable" data-id="<?php echo $class_id; ?>">Delete</a></span></div>
				</div>
			<?php  } ?>
		</div>
	</div>
		...
		<div class="box-body">
			<form class="form-horizontal" id="timetable_form" action="" method="post">
				...
				<div class="form-group" id="enter_sessions">
					...
					<div class="col-md-3" >
						<input type="text" name="noh" class="form-control">
					</div>
					<div class="col-md-3" >
						<span class="text-muted">Include breaks and lunch <br> E.g 8 + 3 = 11 Sessions</span>
					</div>
				</div>
				<div class="col-md-4 col-md-offset-4">
					<input type="submit" Value="Generate" name="stepone" class="btn btn-primary">
				</div>
			</form>
		</div>
	</div>
