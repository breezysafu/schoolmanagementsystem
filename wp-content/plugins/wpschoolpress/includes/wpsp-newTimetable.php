...
$wh_table = $wpdb->prefix . "wpsp_workinghours";
	...
	$skip_hours=TRUE;
		...
		if(empty($sessions)){	
		echo "<div class='alert alert-danger'>No Working Hours added. Please add at <a href='sch-settings/?sc=WrkHours'>Settings</a></div>";
		return false;	
		}	?>	
...
<div class="box box-info">
    ...
    <div class="box-body">
        <?php			if ($sess_template == 'new') {					$session = sanitize_text_field($_POST['session']);			} 
        ...
        <div class="right table-responsive" id="TimetableContainer">
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php						$dayname = array("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday");			
					for ($j = 1; $j <= 7; $j++) {							?>							
                    <tr id="<?php echo $j; ?>">
						...
						if ($hour_det->type == "1") {			
						$td_class = "drop";						
						}										else {											$td_class = "break";										}									$sub_id='';									$sub_name='';									if(isset($timetable[$j][$ses]))										$sub_id=$timetable[$j][$ses];									if($sub_id >0){										$sub_name_f = $wpdb->get_row("SELECT sub_name from $subject_table where id=$sub_id");										$sub_name = $sub_name_f->sub_name;									}									if($sub_name!=''){										$sub_name='<div class="item assigned">'. $sub_name.'</div>';									}else{										$sub_name='';									}									?>									
    ...
    <div class="box-body">
        <form class="form-horizontal" id="timetable_form" action="" method="post">
            ...
            <div class="form-group" id="select_template" style="display:none">
                ...
                <div class="col-md-3" >						<select name="template_class" id="template_class" class="form-control">	<?php
				if (count($avail_sess) > 0) {								foreach ($avail_sess as $avail_clid) {	
				$class_id = $avail_clid->class_id;									echo "<option value='" . $class_id . "'>" . $class_names[$class_id] . "</option>";								}							} else {								echo "<option value=''>No Template Available</option>";							}							?>						</select>					</div>
            </div>
